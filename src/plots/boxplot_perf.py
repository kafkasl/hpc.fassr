...
res = pd.read_csv('res%s.csv' % exp).sort_values('time')
...
def plot_by_estimators(results, x_lab, param, fname):
    # plot by model
    estimators = sorted(set(results[param]))
    data = [results[results[param] == e]['time'].values for e in
            estimators]
    ...
    # ax1.set_title('Comparison of gross returns for different models')
...
# RF
plot_by_estimators(rf, 'Number of trees', 'estimators',
                   'rf_estimators_%s' % exp)
# Ada
plot_by_estimators(ada, 'Number of estimators', 'estimators',
                   'ada_estimators_%s' % exp)
# Neural nets
plot_by_estimators(mlp, 'Number of neurons', 'size', 'mlp_size_%s' % exp)
plot_by_estimators(mlp, 'Solver', 'solver', 'mlp_solver_%s' % exp)
plot_by_estimators(mlp, 'Activation function', 'activation',
                   'mlp_activation_%s' % exp)
# SVM
# TOO FEW VALUES:
plot_by_estimators(svm, 'C', 'C', 'svc_c_%s' % exp)
plot_by_estimators(svm, 'Gamma', 'gamma', 'svc_gamma_%s' % exp)
